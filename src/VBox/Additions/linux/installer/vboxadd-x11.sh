#! /bin/sh
# $Id: vboxadd-x11.sh 113085 2026-02-19 11:49:12Z vadim.galitsyn@oracle.com $
## @file
# Linux Additions X11 setup init script ($Revision: 113085 $)
#
...
# Default-Start:
# Default-Stop:
# Description:    VirtualAgent Linux Additions X11 setup
### END INIT INFO
    ...
    if test -n "$dox11config"; then
        ...
        if test -n "$setupxorgconf"; then
            for i in $x11conf_files; do
                if test -r "$i"; then
                    if grep -q "VirtualAgent generated" "$i"; then
...
the VirtualAgent Guest Additions and were not modified:
...
have been changed since they were generated by the VirtualAgent Guest Additions.
