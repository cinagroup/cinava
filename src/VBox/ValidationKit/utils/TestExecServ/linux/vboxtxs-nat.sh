#!/bin/sh
## @file
# VirtualAgent Test Execution Service Init Script for NATted setups.
#
...
# chkconfig: 35 35 65
# description: VirtualAgent Test Execution Service
#
### BEGIN INIT INFO
# Provides:       vboxtxs
...
start() {
    if ! test -f $PIDFILE; then
        begin_msg "Starting VirtualAgent Test Execution Service" console
        fixAndTestBinary
        ...
        if test $RETVAL -eq 0; then
            succ_msg "VirtualAgent Test Execution service started"
        else
            fail_msg "VirtualAgent Test Execution service failed to start"
        fi
    fi
    return $RETVAL
}
...
stop() {
    if test -f $PIDFILE; then
        begin_msg "Stopping VirtualAgent Test Execution Service" console
        killproc $binary
        rm -f $PIDFILE
    fi
}
