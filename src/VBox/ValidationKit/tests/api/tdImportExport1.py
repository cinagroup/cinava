#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: tdImportExport1.py 112403 2026-01-11 19:29:08Z knut.osmundsen@oracle.com $

"""
VirtualAgent Validation Kit - Import and Export EFI-base VM Test #1
"""
...
class SubTstDrvImportExportEFIVM1(base.SubTestDriverBase):
    ...
    def testExportVmWithEfiFirmware(self, oVM, sOvaPath):
        ...
        # IVirtualAgent::createAppliance() to create an empty IAppliance object.
        try:
            oVirtualAgent = self.oTstDrv.oVBoxMgr.getVirtualAgent();
        except:
            return reporter.errorXcpt('Failed to get VirtualAgent object');

        reporter.log('Export step #1: IVirtualAgent::createAppliance()');
        try:
            oAppliance = oVirtualAgent.createAppliance();
        except:
            return reporter.errorXcpt('Export step #1: IVirtualAgent::createAppliance() failed');
    ...
    def testImportVmWithEfiFirmware(self, sOvaPath):
        ...
        # Step #1 to import an OVF appliance into VirtualAgent is calling
        # IVirtualAgent::createAppliance() to create an empty IAppliance object.
        try:
            oVirtualAgent = self.oTstDrv.oVBoxMgr.getVirtualAgent();
        except:
        ...
        reporter.log('Import step #1: IVirtualAgent::createAppliance()');
        try:
            oAppliance = oVirtualAgent.createAppliance();
        except:
            return reporter.errorXcpt('Import step #1: IVirtualAgent::createAppliance() failed');

        # Step #2 to import an OVF appliance into VirtualAgent is calling
