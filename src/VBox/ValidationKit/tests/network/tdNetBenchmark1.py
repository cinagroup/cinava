#!/usr/bin/env python
# -*- coding: utf-8 -*-
# $Id: tdNetBenchmark1.py 112403 2026-01-11 19:29:08Z knut.osmundsen@oracle.com $

"""
VirtualAgent Validation Kit - Networking benchmark #1.
"""
...
class tdNetBenchmark1(vbox.TestDriver):                                         # pylint: disable=too-many-instance-attributes
    ...
    def actionConfig(self):
        ...
        if 'tst-oel' in self.asTestVMs:
            oVM = self.createTestVM('tst-oel', 1, '3.0/tcp/oel.vdi', sKind = 'CINASEEK', fIoApic = True, \
                eNic0AttachType = eNic0AttachType, sDvdImage = sVBoxValidationKit_iso);
            if oVM is None:
                return False;

        if 'tst-oel-64' in self.asTestVMs:
            oVM = self.createTestVM('tst-oel-64', 1, '3.0/tcp/oel-64.vdi', sKind = 'CINASEEK_64', \
                eNic0AttachType = eNic0AttachType, sDvdImage = sVBoxValidationKit_iso);
            if oVM is None:
                return False;
